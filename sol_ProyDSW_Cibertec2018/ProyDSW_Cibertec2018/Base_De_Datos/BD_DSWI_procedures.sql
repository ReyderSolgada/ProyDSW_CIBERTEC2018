...
IF OBJECT_ID('USP_LISTAR_CLIENTES_X_CODIGO','P') IS NOT NULL
	DROP PROCEDURE USP_LISTAR_CLIENTES_X_CODIGO
GO
CREATE PROCEDURE USP_LISTAR_CLIENTES_X_CODIGO
@XCODCLI CHAR(5)
AS
	SELECT C.CODCLI,C.NOMCLI,D.NOMDIS,C.DIRECCION,C.DNI,C.EMAIL,C.CELULAR,C.ELIMINADO
	FROM TBCLIENTES C INNER JOIN TBDISTRITO D
		ON C.CODDIS = D.CODDIS
	WHERE C.CODCLI=@XCODCLI
GO
